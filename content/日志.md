...
> [!example]- 更新Poetry艺术字组件2.0
>```css title="quartz\plugins\transformers\poetry.ts"
...
>pre.poetry {
>  ...
> // font-family: Kalam, "Zhi Mang Xing";            /* 使用默认艺术文字 也可以不设置 */
>  ...
>}


> [!success]- 完成Poetry可指定艺术字体
> ````md
> ```poetry font="指定艺术字体"
> 
> ```
...
> [!example]- 添加 Poetry 艺术字组件1.0
>> [!TODO]- 第一步：在transformers新建Poetry组件
>>```ts title="quartz\plugins\transformers\poetry.ts"
>> import { QuartzTransformerPlugin } from "../types"
...
>>```
>
>> [!TODO]- 第三步：在quartz.config.ts中导入Poetry组件
...
>> // Example: use custom font for poetry
>> pre.poetry {
>>   font-size: 1.25rem;
...
>>   word-break: break-word;        /* 单词太长时自动断行 */
>>   overflow: visible;             /* 确保不出现滚动条 */
>>   color: var(--dark);            /* 使用主题文字颜色 */
>> }

> [!example]- 引入字体
>> [!TODO]- 引入本地字体
>> ```diff
>> 将 `biro_script_standardRgus` 字体文件和 `font-style.css` 复制到 `quartz/static/font` 目录下，并在 `Head.tsx` 中引入 `font-style.css`。
>>
>> ```diff title="quartz/components/Head.tsx"
>> ...
...
>
>> [!TODO]- 应用字体
>>```diff title="quartz/styles/custom.scss"
>>...
...
> [!Bug]- Bug【已解决】
> 问题：代码过长导致在标注中无法完全显示问题</br>
> 现状：已解决
>```diff title="quartz\styles\custom.scss"
>. . .
>+ // Fix: Ensure code blocks can scroll horizontally when content is too long
