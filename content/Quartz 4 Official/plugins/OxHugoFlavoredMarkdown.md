...
- `replaceOrgLatex`: If `true` (default), converts Org-mode [[Quartz 4 Official/features/Latex|Latex]] fragments to Quartz-compatible LaTeX wrapped in `$` (for inline) and `$$` (for block equations).
